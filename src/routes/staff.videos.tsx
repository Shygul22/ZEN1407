...
function VideosPage() {
  ...
  const { data } = useQuery({
    queryKey: ["videos"],
    queryFn: async () =>
      (await supabase.from("videos").select("*").order("created_at", { ascending: false })).data ??
      [],
  });
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Product videos shown in the customer feed.
          </p>
        </div>
        <UploadDialog />
      </div>
...
function UploadDialog() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!video_url) {
      toast.error("Upload a file or paste a video URL");
      setBusy(false);
      return;
    }
    const { error } = await supabase
      .from("videos")
      .insert({ title, description: desc, video_url, visible, uploaded_by: user?.id });
    setBusy(false);
