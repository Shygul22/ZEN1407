...
function ServicesPage() {
  const { data } = useQuery({
    queryKey: ["services"],
    queryFn: async () => (await supabase.from("services").select("*, customers(full_name, cid)").order("created_at", { ascending: false })).data ?? [],
  });
        ...
        {data?.map((s: any) => (
          <div key={s.id} className="rounded-xl border bg-card p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="font-mono text-[10px] text-muted-foreground">{s.customers?.cid}</div>
      ...
      <div className="hidden overflow-hidden rounded-xl border bg-card md:block">
        <Table>
          <TableHeader><TableRow>
            <TableHead>CID</TableHead><TableHead>Customer</TableHead><TableHead>Issue</TableHead><TableHead>Preferred Date</TableHead><TableHead>Status</TableHead>
          ...
          <TableBody>
            {data?.map((s: any) => (
              <TableRow key={s.id}>
                ...
                <TableCell>{s.issue}<div className="text-xs text-muted-foreground">{s.description}</div></TableCell>
